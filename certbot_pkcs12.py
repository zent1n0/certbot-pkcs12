...
def _load_key(path):
    return serialization.load_pem_private_key(_load_bytes(path), password=None)
...
class Installer(common.Plugin, interfaces.Installer):
    ...
    def deploy_cert(self, domain, cert_path, key_path,
        ...
        pkcs12_data = serialize_key_and_certificates(
            ...
            cert=certificate,
            cas=ca_certificates,
            encryption_algorithm=NoEncryption() if passphrase is None else serialization.BestAvailableEncryption(passphrase)
        )
